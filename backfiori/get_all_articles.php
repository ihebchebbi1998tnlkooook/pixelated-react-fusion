...
try {
    ...
    $itemgroup_product = isset($_GET['itemgroup_product']) ? $_GET['itemgroup_product'] : '';
    $nb_items_passed = isset($_GET['nb_items_passed']) ? intval($_GET['nb_items_passed']) : 0;
    ...
    $sql = "SELECT
                ...
                CASE
                    ...
                    WHEN itemgroup_product IN ('cravates','sacs-a-main', 'portefeuilles', 'porte-cles' , 'porte-passport') THEN
                        qnty_product
                    ELSE
                        (`3xl_size` +`4xl_size` + s_size  + xs_size + m_size + l_size + xl_size + xxl_size)
                END AS qnty_product
            FROM product
            HAVING qnty_product > 0
            ORDER BY RAND()";

    if ($nb_items_passed > 0) {
        $sql .= " LIMIT " . $nb_items_passed;
    }
