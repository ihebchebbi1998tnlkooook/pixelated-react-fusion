        ...
        <div className="container mx-auto px-4">
          <div className="flex flex-col lg:flex-row items-center gap-8">
            ...
            <div className="lg:w-[60%] space-y-8">
              ...
              <div
                ref={statsRef}
                className="grid grid-cols-4 gap-0 pt-6 border-t border-gray-200 w-full md:ml-[10%] ml-[5%] md:ml-[10%]"
              >
                {statistics.map((stat, index) => (
                  <div key={index} className="text-center group mx-auto">
                    <div className="mb-2">
                      {index === 0 && (
                        <Globe className="w-7 h-7 md:w-6 md:h-6 mx-auto text-[#96cc39] group-hover:scale-110 transition-transform" />
                      )}
                      {index === 1 && (
                        <Truck className="w-7 h-7 md:w-6 md:h-6 mx-auto text-[#96cc39] group-hover:scale-110 transition-transform" />
                      )}
                      {index === 2 && (
                        <Award className="w-7 h-7 md:w-6 md:h-6 mx-auto text-[#96cc39] group-hover:scale-110 transition-transform" />
                      )}
                      {index === 3 && (
                        <Factory className="w-7 h-7 md:w-6 md:h-6 mx-auto text-[#96cc39] group-hover:scale-110 transition-transform" />
                      )}
                    </div>
                    <div className="text-[22px] md:text-2xl font-bold text-[#64381b] mb-1 text-center">
                      {Math.floor(stat.current)}
                      {stat.suffix}
                    </div>
                    <div className="text-[12px] md:text-xs text-gray-500 text-center w-full">
                      {index === 0 && "Revendeur"}
                      {index === 1 && "Dattes Exportées"}
                      {index === 2 && "Producteur Affiliés"}
                      {index === 3 && index < statistics.length ? statistics[index].label : ""}
                    </div>
                  </div>
                ))}
              </div>
